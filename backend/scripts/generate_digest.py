...
def render_digest(week_label: str, perf, meta, rows, tables, week_rows_, site_base: str | None) -> str:
    ...
    ap(
        ...
        f"{n_rows} tracked suggestions since inception"
    ...
    for b in BUCKET_ORDER:
        ...
        t10 = (
            f"{_fmt(_cell(tables, 'T+10', b, 'mean'))} ({_cell(tables, 'T+10', b, 'n') or 0})"
        )
        t20 = (
            f"{_fmt(_cell(tables, 'T+20', b, 'mean'))} ({_cell(tables, 'T+20', b, 'n') or 0})"
        )
    ...
    if week_rows_:
        ...
        ap("No tracked suggestions have snapshot dates inside this week — "
    ...
    if site_base:
        ...
        ap("- Full attribution data: /data/performance.json (in-repo: "
