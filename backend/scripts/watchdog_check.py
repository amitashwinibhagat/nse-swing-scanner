...
def parse_iso_utc(s: str) -> Optional[datetime.datetime]:
    try:
...
def read_marker(path: str) -> Optional[datetime.datetime]:
...
def should_trigger(
    *,
    ...
    latest: Optional[dict],
    status: Optional[dict],
    marker_dt: Optional[datetime.datetime],
    now: datetime.datetime,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--run-state", default=RUN_NONE,
